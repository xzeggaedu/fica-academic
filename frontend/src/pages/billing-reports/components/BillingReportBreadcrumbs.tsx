...
interface BillingReportBreadcrumbsProps {
  academicLoadFileId?: number | null;
  isConsolidated?: boolean;
}

export const BillingReportBreadcrumbs: React.FC<BillingReportBreadcrumbsProps> = ({
  academicLoadFileId,
  isConsolidated = false
}) => {
  const navigate = useNavigate();

  // Si es consolidado, solo mostrar hasta "Carga Académica"
  if (isConsolidated) {
    return (
      <Breadcrumb>
        <BreadcrumbList>
          ...
          <BreadcrumbSeparator />
          <BreadcrumbItem>
            <BreadcrumbPage className="text-xs">Consolidado de Planillas</BreadcrumbPage>
          </BreadcrumbItem>
        </BreadcrumbList>
      </Breadcrumb>
    );
