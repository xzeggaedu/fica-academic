...
class TestHourlyRateHistoryValidation:
    ...
    def test_rate_positive_validation(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            HourlyRateHistoryCreate(level_id=1, rate_per_hour=Decimal("-10.00"), start_date=date(2025, 1, 1))
    ...
    def test_rate_zero_validation(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            HourlyRateHistoryCreate(level_id=1, rate_per_hour=Decimal("0.00"), start_date=date(2025, 1, 1))
    ...
    def test_rate_decimal_precision(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            HourlyRateHistoryCreate(level_id=1, rate_per_hour=Decimal("15.999"), start_date=date(2025, 1, 1))
...
class TestHourlyRateHistoryUpdate:
    ...
    def test_update_all_fields(self):
        ...
        assert update.start_date == date(2026, 1, 1)
        assert update.end_date == date(2026, 12, 31)
    ...
    def test_update_date_validation(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            HourlyRateHistoryUpdate(start_date=date(2026, 12, 31), end_date=date(2026, 1, 1))

        assert "debe ser posterior" in str(exc_info.value).lower()

    def test_update_same_dates(self):
        """Prueba que end_date no puede ser igual a start_date."""
        from pydantic import ValidationError

        from src.app.schemas.hourly_rate_history import HourlyRateHistoryUpdate

        with pytest.raises(ValidationError) as exc_info:
            HourlyRateHistoryUpdate(start_date=date(2026, 1, 1), end_date=date(2026, 1, 1))
...
class TestHourlyRateHistoryBase:
    ...
    def test_base_with_end_date(self):
        ...
        assert rate.end_date == date(2025, 12, 31)

    def test_base_without_end_date(self):
...
class TestHourlyRateTimelineItem:
    ...
    def test_timeline_item_inactive(self):
        ...
        assert timeline_item.end_date == date(2024, 12, 31)
